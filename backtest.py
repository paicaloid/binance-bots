    ...
    def __init__(
        ...
        if isinstance(start_date, datetime):
            if time_frame == '1m':
                ...
                self.start_date = start_date - timedelta(hours=self.window_size)
                self.bar_range = int((end_date - start_date).total_seconds() / 3600.0)
        else:
            if time_frame == '1m':
                self.start_date = self.lastest_date - timedelta(minutes=self.bar_range + self.window_size)
            elif time_frame == '1h':
                self.start_date = self.lastest_date - timedelta(hours=self.bar_range + self.window_size)
